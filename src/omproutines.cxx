    ...
    const Int_t numompregions, OMP_Domain *&ompdomain)
{
    ...
    for (i=0;i<numompregions;i++) {
        if ((opt.partsearchtype==PSTALL && opt.iBaryonSearch>1) || (opt.partsearchtype==PSTALLBARYONS && opt.iBaryonSearch>1)) p3dfofomp=tree3dfofomp[i]->FOFCriterionSetBasisForLinks(fofcmp,param,ng,ompminsize,0,0,FOFchecktype, &Head[ompdomain[i].noffset], &Next[ompdomain[i].noffset]);
        ...
        for (i=0;i<numompregions;i++) {
            for (auto j=0;j<omp_nrecv_total[i];j++) {
                ...
                for (auto k=0;k<nt;k++) {
                    ...
                    if ((opt.partsearchtype==PSTALL && opt.iBaryonSearch>1) || (opt.partsearchtype==PSTALLBARYONS && opt.iBaryonSearch>1))
